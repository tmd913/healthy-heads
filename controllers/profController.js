...
module.exports = function (app) {
  ...
  app.get("/prof-finder", isAuthenticated, function (req, res) {
    db.Professional.findAll({}).then(function (profData) {
      ...
      db.Review.findAll({}).then(function (reviewData) {
        hbsObject.professionals.forEach(element => {
          element.reviews = [];
          let reviewCount = 0;
          let ratingSum = 0;
          reviewData.forEach(review => {
            if (review.profID === element.id) {
          ...
          if (reviewCount > 0) {
            element.averageRating = Number.parseFloat(ratingSum / reviewCount).toFixed(1);
          } else {
            element.averageRating = 0;
          }

          console.log(`Avg rating: ${element.averageRating}`);
        });
